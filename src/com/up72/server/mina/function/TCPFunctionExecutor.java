...
//			break;
	...
	private static void changePlayerPais(IoSession session,
				...
				if(room.getPlayerIds().contains(userId))
				{
						...
						for (int i = 0; i < room.getMingPaiInfo().size(); i++) {
							...
							if(room.getPlayerIds().get(i).longValue() == userId.longValue())
							{
								if(room.getMingPaiInfo().get(i) != null &&
										room.getMingPaiInfo().get(i).intValue() == 1)
								{
									//明牌了
									isSendOther = true;
								}
					...
					if(isSendOther)
					{
						for (Long uid : room.getPlayerIds()) {
							...
							if(tmp != null){
								se = MinaServerManager.tcpServer
										.getSessions().get(tmp.getSessionId());
								if (se != null && se.isConnected()) {
									ProtocolData pd = new ProtocolData(interfaceId,
											jsonObj.toJSONString());
									se.write(pd);
	...
	private static void disRoomForce(IoSession session,
					...
					if (lastHeartTime==null) {//说明用户重新上线
						...
						if (roomId!=null) {//用户在房间里
							...
							if (room!=null&&room.getState()!=(Cnst.ROOM_STATE_YJS)) {//房间还没解散
								//toAdd  通知房间内其他人用户上线
								MessageFunctions.interface_100109(RedisUtil.getPlayerList(room.getRoomId(),cid),
...
//					RedisUtil.hset(userKey, "state",Cnst.PLAYER_LINE_STATE_INLINE+"", null);
